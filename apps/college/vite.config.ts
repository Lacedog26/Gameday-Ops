...
// A human-readable build stamp: <pkg version>+<git short sha>, plus the build
// date. Baked in at build time so every deploy is identifiable in the UI and
// a rollback is unambiguous. Falls back gracefully when git isn't available
// (e.g. a source tarball) or on Vercel's shallow checkout.
...
// Vite configuration for GameDayOps College.
