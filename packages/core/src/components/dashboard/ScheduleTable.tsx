...
export default function ScheduleTable({ timeline, colorblind, onAcknowledge }: Props) {
  return (
    <section className="flex min-h-0 flex-1 flex-col">
      {/* Column headings */}
      <div className="grid grid-cols-[8.125rem_5.75rem_1fr_9.375rem_13.125rem] gap-3 px-5 pb-2">
        <Head>CLOCK</Head>
        <Head>T-MINUS</Head>
        <Head>EVENT</Head>
...
function Head({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`font-display text-[1rem] font-bold tracking-[0.3em] text-slate-500 ${className}`}>
      {children}
    </div>
  )
}
